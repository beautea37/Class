...
/*
 * 사용자 정의 예외 : 사용자 정의 예외란 예외 클래스를 프로그래머가 직접 정의해서 던지는 형태를 말합니다.
 * 
 * 이 때 사용되는 형태는 대표적으로 알려진 or 실행예외 두개 정도를 만들어 쓸 수 있습니다.
 * 
 * 알려진 예외는 EXception 의 자식으로 클래스를 정의 하여서 객체를 만들어 사용하는 형태이고
 * 실행예외는 Runtime 을 상속받아 필요시 던지는 형태입니다.
 */
//아래의 클래스는 Exception 을 상소받은 알려진 예외(Checked)로 정의 합니다.
//다른건 다 상속받아서 부모의 메서드를 그대로 이용하면 되지만, 예외의 내용은 직접 정의해야 합니다.
//때문에 파라미터 한개의 생성치를 호출 후 예외 내용을 부모에게 전달하도록 합니다.
...
//계좌 클래스를 정의하고, 인출(withdraw()) 시에 잔액이 부족할 경우 예외가 나오도록 합니다.
